...
class Sandbox(Exchange):
    ...
    def process_order(self, symbol: str, qty: float, price: float, side: str, type: str):
        ...
        if key not in store.positions:
            store.positions[key] = {'qty': 0, 'entry_price': 0}
        ...
        if self.name not in store.balance:
            store.balance[self.name] = 10000 # Default starting balance
            
        if side == 'buy':
            store.balance[self.name] -= (cost + fee)
