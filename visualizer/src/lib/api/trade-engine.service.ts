...
class TradeEngineService {
   ...
   */
  async getDailyPayloads(
    ticker: string,
    interval: string,
    runName: string,
    date: string
  ): Promise<DailyPayloadsResponse> {
    return this.fetchData<DailyPayloadsResponse>('/payloads', {
      ticker,
      interval,
      run_name: runName,
      date,
  ...
  /**
   * Fetches indicators data for a specific backtest run.
   * Corresponds to Python API endpoint `GET /indicators`.
   * @param ticker The currency pair or stock symbol (e.g., `EURUSD=X`).
   * @param interval The candle interval (e.g., `5m`).
   * @param runName The name of the run (e.g., `run-1`).
   * @param key Optional indicator key (case-insensitive) to fetch a specific indicator. If not provided, all indicators are returned.
   * @returns Promise<IndicatorsResponse> The indicators data.
   */
  async getIndicators(
    ticker: string,
    interval: string,
    ...
    const params: Record<string, any> = { ticker, interval, run_name: runName };
    if (key) {
      params.key = key;
    }
    return this.fetchData<IndicatorsResponse>('/indicators', params);
  }
}
