...
{#if s === 'queued' || s === 'processing' || s === 'pending' || s === 'running'}
  ...
  >
    <svg
      class="mr-1.5 h-2 w-2 animate-spin text-blue-400"
...
{:else}
  <span
    class="inline-flex items-center rounded-full bg-slate-800 px-2.5 py-0.5 text-xs font-medium text-slate-400"
  >
    {status}
  </span>
{/if}
